    ...
    async function submitOnMint() {
        ...
        try {
            await window.contractNFT.nft_mint({
                ...
                media: media
            }, 30000000000000, utils.format.parseNearAmount(projectPrice))
        } catch (e) {
            console.log("Error: ", e);
        }
    }
    ...
    return (
        <div>
            <div className='ticket'>
                <h1 className='ticket-tts'>Total NFT Created : {totalTicket}</h1>   
            </div>
            <div className='mint'>
                {console.log(element)}
                <span>NFT' name:</span>
                ...
                <span>Price:</span>
                <Input onChange={(e) => setProjectPrice(e.target.value)} style={{marginBottom: 15}}/>
                ...
                <Button onClick={handleClickMint} className="click-mint">CREATE NFT</Button>
            </div>
        </div>
